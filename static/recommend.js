...
//document.getElementById("enterBtn1").onclick = function() {myFunction()};
//document.getElementById("enterBtn1").onclick = function () { alert('hello!'); };
//function myFunction() {
//    var title = $('.movie').val();
//    console.log('title is : '+title)
//    alert("called")}

function new_movie_recs(movie_title,num_of_recomendation){
  $.ajax({
    type:'POST',
    url:"/similarity",
    data:{'name':movie_title,'nums':num_of_recomendation},
    dataType: 'html',
    success: function(recs){
      if(recs=="Sorry! The movie you requested is not in our database. Please check the spelling or try with some other movies"){
        ...
        $('.fail').css('display','none');
        $('.results').css('display','block');
        console.log(' recommendations aare : '+recs);
        var movie_arr = recs.split('---');
//        console.log(' movie array : '+movie_arr);
        var arr = [];
